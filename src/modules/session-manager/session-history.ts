 ...
 */
export function configureSessionStore(db: Firestore): void {
  firestore = db;
 ...
 */
export async function loadUserHistoryFromStore(userId: string): Promise<void> {
  if (!firestore) return;
  try {
...
export function deleteSession(sessionId: string): void {
  ...
  if (firestore) {
    firestore
      ...
      .catch((error) =>
        logger.error('SessionHistory', 'Failed to delete session from Firestore', { sessionId, error }),
      );
  }
}
...
export function clearHistory(userId: string): void {
  ...
  if (firestore) {
    ...
    db.collection(COLLECTION_NAME)
      ...
      .catch((error) =>
        logger.error('SessionHistory', 'Failed to clear session history in Firestore', { userId, error }),
      );
  }
}
