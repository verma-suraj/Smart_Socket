 ...
 */
function calculatePriorityScore(params: CreateSessionParams): number {
 ...
 */
export function updateSessionEnergy(nodeId: string, powerWatts: number, durationMs: number): void {
  const session = activeSessions.get(nodeId);
  if (!session || !session.active) {
    return;
  }

  const energyKwh = (powerWatts * durationMs) / 3_600_000;
 ...
 */
export function finalizeSession(nodeId: string, reason: SessionEndReason): FinalizedSession {
  const session = activeSessions.get(nodeId);
  if (!session || !session.active) {
 ...
 */
export function getActiveSession(nodeId: string): Session | null {
  const session = activeSessions.get(nodeId);
  if (!session || !session.active) {
    return null;
  }
 ...
 */
export function getActiveSessionsMap(): Map<string, Session> {
