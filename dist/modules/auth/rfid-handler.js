...
export class RfidHandler {
    constructor(firestore, scanModeManager, wsServer) {
     ...
     */
    async authenticate(nodeId, rfidUid) {
        ...
        try {
            ...
            if (result === null) {
                ...
                console.warn(`[AuthModule] Access denied: RFID UID "${rfidUid}" not found (node=${nodeId})`);
                return {
                    success: false,
                    error: 'access_denied',
                    responseTime,
            ...
            if (result === 'TIMEOUT') {
                ...
                console.error(`[AuthModule] Auth timeout: RFID lookup exceeded ${RFID_AUTH_TIMEOUT_MS}ms (node=${nodeId}, uid=${rfidUid})`);
                return {
                    success: false,
                    error: 'auth_timeout',
                    responseTime,
            ...
            // Success — user found
            console.log(`[AuthModule] Auth success: user=${result} authenticated at node=${nodeId}`);
            return {
                success: true,
                userId: result,
                responseTime,
            ...
            console.error(`[AuthModule] Auth error for node=${nodeId}, uid=${rfidUid}: ${message}`);
            return {
                success: false,
...
//# sourceMappingURL=rfid-handler.js.map