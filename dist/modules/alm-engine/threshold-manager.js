...
export class ThresholdManager {
     ...
     */
    async setThreshold(value, updatedBy = 'system') {
        ...
        console.log(`[ThresholdManager] Threshold updated to ${value}W by ${updatedBy}`);
    }
    /**
        ...
        try {
            ...
            if (doc.exists) {
                ...
                if (data && typeof data.loadThreshold === 'number') {
                    ...
                    console.log(`[ThresholdManager] Loaded threshold from Firestore: ${this.threshold}W`);
                    return;
                }
            }
            // No persisted value found — use default
            this.threshold = config.alm.defaultThreshold;
            console.log(`[ThresholdManager] No persisted threshold found, using default: ${this.threshold}W`);
        }
        catch (error) {
            // On Firestore failure, retain the default threshold
            console.error('[ThresholdManager] Failed to load threshold from Firestore, using default:', error);
            this.threshold = config.alm.defaultThreshold;
        }
    }
}
/** Firestore collection name for system configuration. */
ThresholdManager.CONFIG_COLLECTION = 'config';
/** Firestore document ID for the ALM config entry. */
ThresholdManager.CONFIG_DOC_ID = 'alm';
//# sourceMappingURL=threshold-manager.js.map